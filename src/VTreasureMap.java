...
public class VTreasureMap {
	...
	private VSectionManager sectionManager;
	
	private VSection viewArea; //the area that has been discovered
	...
	public VTreasureMap(){
		...
		sectionManager = new VSectionManager();
		viewArea = new VSection(dimensions);
	}
	...
	private void addToSection(Integer[] p, char c) {
		...
		if(c != '.'){
			viewArea.setValue(p[0], p[1], c);
			viewArea.setDimensions(dimensions);
		}
		if(c == ' ' || c == 'a' || c == 'k' || c == 'd' || c == '$' || c == '^' || c == 'T'){
			sectionManager.addLand(p, dimensions, c);
		}else if(c == '~'){
			sectionManager.addWater(p, dimensions, c);
		}
		sectionManager.setAllDimensions(dimensions);
	}
	public void addToLand(int[] p, char c) {
		Integer i[] = {p[0], p[1]};
		sectionManager.addLand(i, dimensions, c);
		//sectionManager.setAllDimensions(dimensions);
	}
	public VSectionManager getSectionManager(){
	...
	public boolean isBlockedAt(int x, int y, boolean hasBoat, boolean hasKey, boolean onWater){
		char c = map[y+centre][x+centre];
		if (onWater) {
			return c == '*' || c == '.' || c == 'T' || (!hasBoat && c == '~') || (!hasKey && c == '-') || c == ' ';
		} else {
			return c == '*' || c == '.' || c == 'T' || (!hasBoat && c == '~') || (!hasKey && c == '-') ;
		}
	}
